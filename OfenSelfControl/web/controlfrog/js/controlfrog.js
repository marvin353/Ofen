...
$(document).ready(function() {
	...
	/*
	*	Copy the each() function for each line chart you
	...
	*/	
	$('.cf-line').each(function(){
		...
		*/
		...
		var cId = $(this).prop('id');					// Chart container id e.g. cf-line-1
          ...
          datasets: [
            {
              ...
              borderColor: "red",//line.line,
              pointBackgroundColor: line.pointBackgroundColor,
              pointBorderColor: "red",//line.pointBorderColor,
			  pointRadius:0,
              data: [40, 20, 12, 39, 10, 40, 39, 80, 40, 20, 12, 11,66,77,88,99,77,77,77,77,77]
            }
          ]
				},
				options: lineOptions
		});
		//addData(cf_lCrt[cId], 'Jan', 100);
		//removeData(cf_lCrt[cId]);
		//removeData(cf_lCrt[cId]);
		//updateDataset("cf-line-1");
	});
});

function updateChart(chart, opts){
	/*if(opts.minVal){
...
function addData(chart, label, data) {
    cf_lCrt[chart].data.labels.push(label);
    cf_lCrt[chart].data.datasets.forEach((dataset) => {
        dataset.data.push(data);
    });
    cf_lCrt[chart].update();
}

function updateDataset(chart,labels, data) {
	console.log("updateDataset");
	
    cf_lCrt[chart].data.labels = labels;//["a","d","c"];
    cf_lCrt[chart].data.datasets.forEach((dataset) => {
        dataset.data= data//[1,2,3]
    });
    cf_lCrt[chart].update();
}

function removeData(chart) {
    cf_lCrt[chart].data.labels.pop();
    cf_lCrt[chart].data.datasets.forEach((dataset) => {
        dataset.data.pop();
    });
    cf_lCrt[chart].update();
					...
					datasets: [
						{
							...
							pointRadius: 1,
							data: [1000, 200, 500, 900, 500, 100, 300, 500, 50, 150, 100, 80, 200, 900, 450]
						}
					],
				},
...
$(document).ready(function(){
	...
	$('.cf-zoned-gauge').each(function(){
		...
		//rZGopts.colorStart = "blue";
		//rZGopts.colorStop = "red";
...
function colorCodeGauge(temp) {
			...
			return colorHSL = "hsla(" + HValue + ", 100%, 50%, 1)";
